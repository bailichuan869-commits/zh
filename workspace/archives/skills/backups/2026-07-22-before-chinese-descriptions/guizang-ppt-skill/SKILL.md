...
- 如果当前运行环境支持结构化提问或表单式澄清工具,可以用它们逐项确认风格、受众、素材和截图需求。
- 如果当前运行环境不支持这类工具,就用普通对话直接询问用户。一次最多问 1-3 个最关键问题;如果信息缺口不影响开工,先做合理假设并在回复里说明。
...
1. 同时打开原始参考 PPT、当前模板或生成页、测试 PPT;原始参考应使用 skill 自带的模板或当前项目里已存在的参考 deck，不要依赖作者本机的绝对路径。
