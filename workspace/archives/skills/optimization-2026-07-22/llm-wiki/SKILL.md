...
**CHECKPOINT:** Before modifying an existing wiki in interactive mode, show the source identity, pages expected to change, and any detected contradictions. Wait for confirmation when the source is marked important/critical, when more than 10 existing pages would change, or when a claim would replace a newer sourced claim.
...
|---|---|---|
| `WIKI.md` or `wiki/index.md` is missing | Inspect the project and offer to initialize the missing structure | Stop before writing and show the exact files that would be created |
| A source file cannot be read | Record the source as pending and continue only with readable sources | Do not create claims or summaries for the unreadable source |
| New and existing sources contradict | Preserve both claims with citations and dates; flag the conflict | Ask the user which interpretation should drive synthesis, without deleting either source record |
| A referenced page does not exist | Create a missing-page lint item instead of an empty authoritative page | Create the page only when enough sourced content is available |
| `qmd` is unavailable | Use `wiki/index.md` and runtime-native file search | Report reduced retrieval coverage if the wiki is too large to scan reliably |
| A write fails partway through ingest | Stop, list files already changed, and avoid updating `log.md` as if complete | Resume from the checklist after the user or runtime resolves the write issue |
...
- Do not modify files under `raw/`; treat source documents as immutable.
- Do not merge contradictory claims into a false consensus or silently discard the older claim.
- Do not create citations, entities, dates, links, or source relationships that are not supported by the files read.
- Do not write query answers back to the wiki without user confirmation.
- Do not mark an ingest complete until the source summary, index, cross-references, and log agree.
