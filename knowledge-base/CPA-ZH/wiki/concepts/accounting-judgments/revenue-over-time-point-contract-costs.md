...
用于判断与“收入按时段或时点确认及合同成本”直接相关的确认、计量、列报和披露问题。涉及税务、法律可执行性、估值或其他准则交叉事项时，应同时取得相应专业证据，不能把本专题替代具体合同和最新有效准则。

## 决定性事实

客户是否同步取得利益、在建资产控制、替代用途、可执行收款权、履约进度、增量取得成本和履约成本可收回性。信息不足时，应先补齐这些事实并给出条件化回答，不强行输出唯一结论。
