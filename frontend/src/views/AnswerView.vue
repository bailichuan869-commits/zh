...
import { api, type AnswerResult } from '@/services/api'
...
async function ask() {
  ...
  try { result.value = await api.answer(question.value.trim(), topic.value) } catch (reason) { error.value = reason instanceof Error ? reason.message : '答疑服务暂时不可用' } finally { loading.value = false }
}
...
function openCitation(path: string) { router.push({ path: path.startsWith('wiki/') ? '/document' : '/raw', query: { path } }) }
</script>
...
<template>
  <section class="answer-page">
    ...
    <a-card :bordered="false" class="question-card">
      <a-form layout="vertical" @finish="ask"><a-form-item label="问题"><a-textarea v-model:value="question" :rows="4" placeholder="例如：含可变付款的售后租回应如何判断？" /></a-form-item><a-form-item label="主题范围"><a-select v-model:value="topic" allow-clear placeholder="自动从全部已复核主题中检索"><a-select-option v-for="item in topics" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select></a-form-item><a-button type="primary" html-type="submit" :loading="loading"><SendOutlined />获取有依据的答复</a-button></a-form>
    </a-card>
    <a-alert v-if="error" class="space" type="error" show-icon :message="error" />
    <a-spin :spinning="loading"><section v-if="hasResult" class="space"><a-alert v-if="result?.insufficient_evidence" type="warning" show-icon message="现有已复核资料不足" description="系统未生成专业结论；请先核验下方资料或补充复核知识页。" /><a-card v-else :bordered="false" class="answer-card"><template #title>答复</template><div class="answer-text">{{ result?.answer }}</div><a-tag color="blue">{{ result?.confidence }}</a-tag></a-card><a-card :bordered="false" class="citations-card" title="证据与原文"><a-empty v-if="!result?.citations.length" description="未找到可展示的相关资料" /><a-list v-else :data-source="result?.citations"><template #renderItem="{ item }"><a-list-item><a-list-item-meta :title="item.title" :description="item.excerpt" /><template #actions><a @click="openCitation(item.path)"><LinkOutlined />打开</a></template><a-space><a-tag v-if="item.answer_ready" color="green">已复核</a-tag><a-tag v-if="item.authority">{{ item.authority }}</a-tag><a-tag v-if="item.maturity">{{ item.maturity }}</a-tag></a-space></a-list-item></template></a-list></a-card></section></a-spin>
  </section>
</template>
