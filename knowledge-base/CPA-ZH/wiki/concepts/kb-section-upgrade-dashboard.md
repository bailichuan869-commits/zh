...
| 板块 | 页面数 | 主要治理目标 | 当前提示 |
|---|---:|---|---|
| 一、行业重要法规与准则 | 183 | 官方来源、原文归档、本地索引三者一致；核心法律、会计准则、审计准则页面保留效力状态和更新时间；将高频准则继续沉淀为实务专题、审计程序和底稿提示 | 无 |
| 二、行业重要政策性文件 | 15 | 政策原文、版本效力、执行检查清单持续联动；按月或遇监管新规时复核官方链接和有效状态；把政策要求拆成事务所治理、项目执行、人员管理和监督检查动作 | 无 |
...
| 五、AI 编程与自动化 | 17 | 讲义保留原始 Markdown，wiki 层提炼学习线和工具模板；按 Agent、Python、VBA、审计自动化场景维护索引；记录自动化工具的数据边界、控制点、证据留痕和适用风险 | 无 |
| 维护、索引与来源页 | 21 | 保持元数据、来源和索引可追溯 | 无 |
