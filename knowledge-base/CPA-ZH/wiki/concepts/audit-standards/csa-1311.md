---
title: 中国注册会计师审计准则第1311号——对存货、诉讼和索赔、分部信息等特定项目获取审计证据的具体考虑
type: concept
concept_type: audit-standard
created: 2026-06-26
updated: 2026-06-26
sources: [cicpa-professional-standards-number-index-2026-06-26]
tags: [audit, standards, cicpa, p1-core]
related: [[concepts/audit-standards-system]], [[sources/cicpa-professional-standards-number-index-2026-06-26]]
domain: audit-standards
topic: csa
---

# 中国注册会计师审计准则第1311号——对存货、诉讼和索赔、分部信息等特定项目获取审计证据的具体考虑

## 定位

- 准则类型：审计准则
- 索引键：`csa-1311`
- 资料记录数：1

## 资料分布

| 类型 | 数量 |
|---|---:|
| 应用指南 | 1 |

## 关联资料

| 类型 | 标题 | 来源分组 | 官方链接 | 本地文件 |
|---|---|---|---|---|
| 应用指南 | 《中国注册会计师审计准则第1311号——对存货、诉讼和索赔、分部信息等特定项目获取审计证据的具体考虑》应用指南 | 2023-34项审计准则应用指南 | https://www.cicpa.org.cn/xxfb/tzgg/202304/W020230410260829902598.pdf | `raw/standards/audit/pdfs/042-中国注册会计师审计准则第1311号-对存货-诉讼和索赔-分部信息等特定项目获取审计证据的具体考虑-应用指南.pdf.md` |
