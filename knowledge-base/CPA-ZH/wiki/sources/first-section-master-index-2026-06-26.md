---
title: 第一板块资料总表
type: source
source_type: master-index
created: 2026-06-26
updated: 2026-07-14
sources: [first-section-master-index-2026-06-26, challenge-knowledge-source-summary-2026-07-13]
raw_path: raw/indexes/first-section-master-index.csv
tags: [cpa, law, accounting, audit, standards, master-index, p1-core]
related: [[concepts/regulations-and-standards]], [[concepts/accounting-standards-system]], [[concepts/audit-standards-system]]
domain: sources
topic: batches
---

# 第一板块资料总表

## 文件位置

- CSV 明细：`raw/indexes/first-section-master-index.csv.md`
- Markdown 总览：`raw/indexes/first-section-master-index.md`

## 汇总口径

本总表汇总“行业重要法规与准则”板块已纳入 raw 层的资料，覆盖四部核心法律、企业会计准则体系资料，以及中国注册会计师执业准则及应用指南资料。

2026-08-05 维护说明：根据 [[sources/challenge-knowledge-source-summary-verification-2026-07-13]]，本表中的《中华人民共和国注册会计师法》当前先纳入 2026 修订草案 60 条过渡文本；草案非官方重新公布全文，正式引用前仍需核对官方重排版。

| 类别 | 数量 | 状态 |
|---|---:|---|
| 核心法律 | 4 | 已纳入本地 raw 层；《注册会计师法》当前为 2026 修订草案过渡文本，待官方重排全文核对 |
| 企业会计准则 | 47 | 已下载财政部会计司 HTML 原文页 |
| 企业会计准则解释 | 20 | 已下载财政部会计司 HTML 原文页 |
| 企业会计准则其他规定 | 22 | 已下载财政部会计司 HTML 原文页 |
| 企业会计准则应用案例 | 63 | 已下载财政部会计司 HTML 原文页 |
| 企业会计准则实施问答 | 163 | 已下载财政部会计司 HTML 原文页 |
| 中国注册会计师执业准则及应用指南 PDF/ZIP 解压文件 | 85 | 已下载或解压中注协附件 |
| 中注协执业准则专题条目 | 72 | 已建立索引 |
| 合计 | 476 | 第一板块 raw 层总表 |

## 用途

该总表用于快速核对第一板块资料是否已取得、官方链接是否可追溯、文件在 raw 层的位置是否明确。后续将基于该总表推进条款级和准则编号级 Markdown 化。

## 后续处理

1. 将四部核心法律拆分为条款级知识页，并补充官方版本核验信息。
2. 将企业会计准则、解释、应用案例、实施问答映射到准则编号和主题。
3. 提取中注协 PDF 文本，建立“准则编号 - 名称 - 适用业务 - 应用指南”索引。
