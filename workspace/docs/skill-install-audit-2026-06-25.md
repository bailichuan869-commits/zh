...
**日期**：2026-06-25
**操作**：将 `materials/skill_archives/` 中保留的 8 个 skill 压缩包解包、安全审计后安装到用户级目录 `~/.workbuddy/skills/`
...
**全部 8 个保留 skill 风险等级为 P2（安全），可正常安装。**
...
安装位置：`%USERPROFILE%\.workbuddy\skills\`

| 序号 | Skill 名称 | 来源压缩包 | 用途 |
|------|------------|------------|------|
| 1 | course-generator | course_generator_75e023cd50.zip | 零基础课程生成 |
| 2 | darwin-skill | darwin_skill_3dfbee3ea1.zip | Skill 质量评估与优化 |
| 3 | find-skills | find_skills_0_1_0_9a49ce6b49.zip | 发现并安装新 skill |
| 4 | guizang-ppt-skill | guizang_ppt_skill_235d3f3e54.zip | 网页 PPT 生成 |
| 5 | llm-wiki | llm_wiki_1_0_1_259c6dd94f.zip | 个人知识库管理 |
| 6 | multi-search-engine | multi_search_engine_2_1_3_b32487e0d2.zip | 多搜索引擎集成 |
| 7 | regulatory-penalty-evaluator | regulatory_penalty_evaluator_55d678cff6.zip | 监管处罚风险评估 |
| 8 | tianchuan-audit-perspective | tianchuan_audit_perspective_36243a14a9.zip | 田川审计底稿视角 |
...
8 个保留 skill 的 `SKILL.md` 全部存在性验证通过 `[OK]`。连同原有的 `qcc-company`、`travel-cn`，用户级 skill 目录现有 10 个非系统 skill。
